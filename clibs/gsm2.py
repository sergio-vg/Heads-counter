...
class sim():
    ...
    def sendCommand(self, command, start_string, succ_string, at_flag, sent_flag, expeca1, expeca2, mqttString):
        ...
        if self.commandSent == False:
            ...
            if self.acheck != 0:
                self.time = float((datetime.now().strftime("%S.%f"))[:-3])
                

        return self.commandSent, self.atflag, self.time, self.acheck;
    ...
    def mqttPublish(self, stringMessage, pubPackId):
        ...
        self.time2 = float((datetime.now().strftime("%S.%f"))[:-3])
        self.t0 = None 
        self.an = None
        self.c = 0
        ...
        while True:
            ...
            if self.ATFlag == 0:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("ATE0&W", "Disabling Echo...", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 1:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CFUN=1", "Waking up GSM Module...", None, self.ATFlag, self.sentFlag, "Call Ready", None, None)

            elif self.ATFlag == 2:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPSHUT", "Closing previous GPRS...", None, self.ATFlag, self.sentFlag, "SHUT", "OK", None)

            elif self.ATFlag == 3:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPMUX=0", "Configuring single ip connection...", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 4:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPRXGET=1", "Getting data from network manually...", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 5:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CGATT=1", "GPRS Service's status...", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 6:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CSTT=" + self.APN, "Setting APN...", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 7:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIICR", "Bring up wireless connection...", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 8:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIFSR", "Getting local ip adress...", None, self.ATFlag, self.sentFlag, ".", None, None)

            elif self.ATFlag == 9:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPSTART=" + self.TCP_DIR, "Startup connection...", None, self.ATFlag, self.sentFlag, "CONNECT", None, None)

            elif self.ATFlag == 10:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPSEND", "Preparing for send data...", None, self.ATFlag, self.sentFlag, ">", None, None)

            elif self.ATFlag == 11:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("connect", None, None, self.ATFlag, self.sentFlag, "SEND", "OK", None)

            elif self.ATFlag == 12:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPRXGET=2,1024", None, "Connected to MQTT broker.", self.ATFlag, self.sentFlag, " \x02\x00\x00", None, None)
                if self.an is not None and self.an == 1:
                    print("conectado")
                    while self.c <= 3:
                        GPIO.output(24, 0)
                        time.sleep(.12)
                ...
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("publish", None, None, self.ATFlag, self.sentFlag, "SEND", "OK", self.strMsg)

            elif self.ATFlag == 15:
                self.sentFlag, self.ATFlag, self.t0, self.an = self.sendCommand("AT+CIPRXGET=2,1024", None, "Published succesfully to MQTT broker.", self.ATFlag, self.sentFlag, self.pubExp, None, None)
                if self.an is not None and self.an == 1:
                    print("publicado")
                    while self.c <= 5:
                        GPIO.output(24, 0)
                ...
                self.sentFlag, self.ATFlag, self.t0, self.an,  = self.sendCommand("AT+CFUN=0", "Putting GSM module to sleep... ", None, self.ATFlag, self.sentFlag, "OK", None, None)

            elif self.ATFlag == 17:
                break

            self.time1 = float((datetime.now().strftime("%S.%f"))[:-3])

            if self.t0 is not None:
                self.time2 = self.t0

            if (self.time1 > self.time2 and (self.time1 - self.time2) > self.maxtime) or (self.time1 < self.time2 and (60.0 - self.time2 + self.time1) > self.maxtime):
                if self.ATFlag != 15 and self.ATFlag != 12:
                    self.sentFlag = False
                    self.time2 = float((datetime.now().strftime("%S.%f"))[:-3])
                    self.failed_attempts += 1                    
                    print("Sending command again")

                    if self.failed_attempts >=4:
                        self.ATFlag = 16
                        print("exceso de intentos")

            elif self.ATFlag == 12 and self.an != 1 and self.an is not None:
                print("Trying to connect again to MQTT Broker...")
                self.sentFlag = False
                self.time2 = float((datetime.now().strftime("%S.%f"))[:-3])
                self.ATFlag = 10

            elif self.ATFlag == 15 and self.an != 1 and self.an is not None:
                print("Trying to publish again to MQTT Broker...")
                self.sentFlag = False
                self.time2 = float((datetime.now().strftime("%S.%f"))[:-3])
                self.ATFlag = 13
